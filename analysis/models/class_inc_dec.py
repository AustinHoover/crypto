...
timeWindowSizeOut = 1
normalizationConstant = 800 # Constant to scale the dataset by to normalize data for easier training
...
epochs = 25 # number epochs to train
...
inFrame: pd.DataFrame = pd.read_csv('../datasets/Augmented.csv')
...
# resolve series
originalSeriesToPredict: pd.Series = inFrame[predictColumn]
seriesToPredictList = inFrame[predictColumn].tolist()
savgol = scipy.signal.savgol_filter(seriesToPredictList[1:],25,3)
print(savgol)
seriesInput = [*['Close'],*savgol]
fitleredData = pd.Series(seriesInput)
seriesToPredict = originalSeriesToPredict
# time shift
timeShiftedSeriesToPredict: pd.Series = seriesToPredict.diff().transform(lambda x: 1 if x > 0 else 0).shift(periods=-timeWindowSizeOut) # shift it back one window
# construct new frame                                                                    remove NaN          Chop off end to line up sets
unNormalizedTimeAdjustedOutFrame : pd.DataFrame = pd.DataFrame({predictColumn:timeShiftedSeriesToPredict[(timeWindowSizeIn * 2):-timeWindowSizeOut]})
# difference the out frame
# differencedOutFrame = unNormalizedTimeAdjustedOutFrame.diff()
# then normalize
oneHotEncodedOutFrame = pd.get_dummies(unNormalizedTimeAdjustedOutFrame[predictColumn])
timeAdjustedOutFrame = oneHotEncodedOutFrame
# print(pd.get_dummies(timeAdjustedOutFrame['Close']))
...
model.add(layers.Dense(expectedOutShape[0] * expectedOutShape[1] * 8, activation='relu'))

model.add(layers.Dense(expectedOutShape[0] * expectedOutShape[1], activation='softmax'))
...
checkpoint = ModelCheckpoint(
    "../results/class_inc_dec_best_model.hdf5",
...
# Save the model
# Deprecated: this is accomplished with checkpoint code from above
