...
#Neural Net
#
magnitudeModelPath = '../analysis/results/magnitude_best_model.hdf5'
magnitudeModel = keras.models.load_model(magnitudeModelPath)
magnitudeModel.summary()

traditionalModelPath = '../analysis/results/best_model.hdf5'
traditionalModel = keras.models.load_model(traditionalModelPath)
traditionalModel.summary()
...
def eval():
    ...
    # run NN and extract from inner arrays
    magnitudes = list(map(lambda x: x[0], magnitudeModel.predict(NNinput)[0]))
    classes = list(map(lambda x: x[0], traditionalModel.predict(NNinput)[0]))
    print("classes")
    print(classes)
    #bind the two arrays together into final result array
    result = []
    for i in range(0,len(magnitudes)):
        if(classes[i] > 0):
            result.append(0.5 + magnitudes[i])
        else:
            result.append(0.5 - magnitudes[i])
    print(result)
