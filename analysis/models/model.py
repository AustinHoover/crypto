...
model = keras.Sequential(
    [
        keras.Input(shape=(rowsToTrain + valuesPerTimestamp)),
        # layers.Flatten(),
        # layers.Dropout(0.5),
        layers.Dense(rowsToTrain + valuesPerTimestamp,activation="softmax"),
...
for epoch in range(1,epochs):
    ...
    for i in range(parsingStart,parsingEnd):
        ...
        currentTrainPool.append(currentFrame[4])
        #print(currentFrame)
    #must fill trainPool with dummy values for beginning
    for i in range(0,valuesPerTimestamp):
        currentTrainPool.append(1)
    ...
    for i in range(parsingStart,parsingEnd):
        #currentTrainPool
        nextTrainRow = currentTestPool.pop(0)
        currentTrainPool = currentTrainPool[1:-(valuesPerTimestamp-1)]
        currentTrainPool = currentTrainPool + [nextTrainRow[4]] + nextTrainRow[0:4] + nextTrainRow[5:]
        #currentTestPool
        currentRow = next(parsedData)
        currentFrame = list(map(lambda x: float(x),currentRow))
        currentTestPool.append(currentFrame)
